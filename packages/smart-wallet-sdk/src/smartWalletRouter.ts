...
import {
      ...
      encodeOperation,
} from "./encoder/walletOperations";
import {
      type PermitWithWithWitness,
      permit2TpedData,
} from "./permit/permit2TypedData";
...
import {
      ...
      getTokenPriceByNumber,
      getUsdGasToken,
} from "./utils/estimateGas";
...
export abstract class SmartWalletRouter {
      ...
      public static tradeConfig = {} as Partial<ClassicTradeOptions<any>> &
      ...
      public static buildClassicTrade<
            UTradeOps extends SwapOptions & PancakeSwapOptions,
      >(
            ...
            if (options.router === Routers.UniversalRouter) {
                  const { value, calldata } =
                        UniversalRouter.swapERC20CallParameters(trade, routeOptions);
                  const swapRouterAddress = getUniversalRouterAddress(
                        options.chainId
                  );
      ...
      public static buildSmartWalletTrade(
            trade: SmartRouterTrade<TradeType>,
      ...
      public static async estimateSmartWalletFees({
            ...
            if (!nativeWrappedToken) {
                  throw new Error(
                        `Unsupported chain ${chainId}. Native wrapped token not found.`
                  );
            }
            ...
            const quotePriceInUsd = getTokenPriceByNumber(
                  usdToken,
                  outputCurrency,
                  quoteCurrencyUsdPrice?.priceUSD
            );
            const basePriceInUsd = getTokenPriceByNumber(
                  usdToken,
                  inputCurrency,
                  baseCurrencyUsdPrice?.priceUSD
            );
            ...
            const quotePriceInNative =
                  quotePriceInUsd && nativePriceInUsd
                        ? nativePriceInUsd.multiply(quotePriceInUsd.invert())
                        : undefined;

            const basePriceInNative =
                  basePriceInUsd && nativePriceInUsd
                        ? nativePriceInUsd.multiply(basePriceInUsd.invert())
                        : undefined;

            let tradeGasEstimation =
                  trade?.gasEstimate ?? (trade as any)?.gasUseEstimate ?? 0n;
            ...
            if (options.isUsingPermit2) {
                  await contract.estimateGas
                        .transferFrom(
                              [address, options.smartWalletDetails.address, 0n],
            ...
            await contract.estimateGas
                  .approve([SMART_ROUTER_ADDRESSES[97], MaxUint256], {
                        account: signer.address,
                  })
                  .then((gas) => {
                        tradeGasEstimation += gas;
            ...
            await contract.estimateGas
                  .transfer([signer.address, trade.inputAmount.quotient], {
                        account: address,
                  })
                  .then((gas) => {
                        tradeGasEstimation += gas;
            ...
            const baseGasCostWei =
                  gasPrice * (tradeGasEstimation + estimationOfSmartWalletBatchExec);
            const totalGasCostNativeCurrency = CurrencyAmount.fromRawAmount(
                  nativeWrappedToken,
                  baseGasCostWei
            );

            let gasCostInQuoteToken: CurrencyAmount<Currency> =
                  CurrencyAmount.fromRawAmount(outputCurrency, 0n);
            let gasCostInBaseToken: CurrencyAmount<Currency> =
                  CurrencyAmount.fromRawAmount(outputCurrency, 0n);
            let gasCostInUSD: CurrencyAmount<Currency> =
                  CurrencyAmount.fromRawAmount(usdToken, 0n);
            ...
            if (
                  ...
                  gasCostInQuoteToken = quotePriceInNative.quote(
                        totalGasCostNativeCurrency
                  );
                  gasCostInBaseToken = basePriceInNative.quote(
                        totalGasCostNativeCurrency
                  );
            }
      ...
      public static encodeSmartRouterTrade(args: [UserOp[], Hex], to: Address) {
            const { encodedSelector, encodedInput } = encodeOperation(
                  OperationType.EXEC,
                  args
            );
            const callData = encodedSelector.concat(
                  encodedInput.substring(2)
            ) as Hex;
      ...
      public static async getUserSmartWalletDetails(
            ...
            const address = await factory.read.walletAddress([
                  userAddress,
                  BigInt(0),
            ]);
