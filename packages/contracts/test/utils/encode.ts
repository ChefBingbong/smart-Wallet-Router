...
export function legacyTopology(
  leavesOrConfig: SimplifiedWalletConfig | ConfigTopology[],
): ConfigTopology {
...
export function toTopology(
  config: SimplifiedWalletConfig | SimplifiedNestedWalletConfig,
): ConfigTopology[] {
...
export function merkleTopology(
  leavesOrConfig: SimplifiedWalletConfig | ConfigTopology[],
): ConfigTopology {
  if (!Array.isArray(leavesOrConfig)) {
...
export function optimize2SignersTopology(
  config: SimplifiedWalletConfig,
): ConfigTopology {
...
export function imageHash2(
  threshold: ethers.BigNumberish,
  topology: ConfigTopology,
): string {
  const root = hashNode(topology);
  return ethers.utils.keccak256(
    ethers.utils.solidityPack(["bytes32", "uint256"], [root, threshold]),
  );
}

export function printTopology(
  topology: ConfigTopology,
  threshold?: ethers.BigNumberish,
  inverse = false,
): string[] {
  ...
  if (isNestedLeaf(topology)) {
    const result: string[] = [
      `internalThreshold: ${topology.internalThreshold} - externalWeight: ${topology.externalWeight}`,
    ];
...
export function encodeNonce(space: BigNumberish, nonce: BigNumberish) {
  return ethers.BigNumber.from(
    ethers.utils.solidityPack(["uint160", "uint96"], [space, nonce]),
  );
}

export function leafForAddressAndWeight(
  address: string,
  weight: ethers.BigNumberish,
) {
  return ethers.utils.solidityPack(["uint96", "address"], [weight, address]);
}
...
export function subdigestOf(
  wallet: string,
  digest: ethers.BytesLike,
  chainId: ethers.BigNumberish = 31337,
) {
  return ethers.utils.keccak256(
    ethers.utils.solidityPack(
...
export class SignatureConstructor {
  ...
  tryTrim(): void {
    ...
    if (
      first.type !== SignaturePartType.Address &&
      first.type !== SignaturePartType.Node
    )
      return;
    if (
      second.type !== SignaturePartType.Address &&
      second.type !== SignaturePartType.Node
    )
      return;

    const firstNode =
      first.type === SignaturePartType.Address
        ? leafForAddressAndWeight(first.address!, first.weight!)
        : first.value;
    const secondNode =
      second.type === SignaturePartType.Address
        ? leafForAddressAndWeight(second.address!, second.weight!)
        : second.value;
    ...
    switch (part.type) {
      case SignaturePartType.Address:
        this.members.push({
          ...
          type: SignaturePartType.Address,
        });
        break;
  ...
  for (const node of slice) {
    if (!isConfigLeaf(node)) {
      ...
      if (isSignerLeaf(node)) {
        ...
        const part = parts.get(node.address) ?? {
          type: SignaturePartType.Address,
...
export function encodeSignature(
  ...
  const encodedSigners = encodeSigners(
    config.topology,
    parts,
    subdigests,
    options,
